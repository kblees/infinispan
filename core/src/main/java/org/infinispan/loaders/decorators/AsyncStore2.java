 ...
 * @since 4.0
 */
public class AsyncStore2 extends AbstractDelegatingStore {
   ...
   public AsyncStore2(CacheStore delegate, AsyncStoreConfig asyncStoreConfig) {
      super(delegate);
   ...
   private class AsyncStoreProcessor implements Runnable {
      ...
      private void retryWork(int maxRetries) {
         for (int attempt = 0; attempt < maxRetries; attempt++) {
            ...
            try {
               AsyncStore2.this.applyModificationsSync(modifications);
               return;
            } catch (Exception e) {
               if (log.isDebugEnabled())
                  log.debug("Failed to process async modifications", e);
